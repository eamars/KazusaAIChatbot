...
This roster is complete for the current native runtime. The registry is the
declarative schema/permission/prompt/handler authority for all seven kinds.
`apply_memory_lifecycle_update` is internal-only. Accepted-task control, status,
...
## Runtime Registry Commitments

These entries are authoritative for the current native execution surface.
