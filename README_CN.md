...
除此之外，系统还需要一个 OpenAI 兼容的嵌入向量接口，用于历史对话、长期记忆和向量检索。本地部署通常可以使用 LM Studio，也可以换成其他兼容的大语言模型后端服务 （比如Ollama）。
...
| 认知与对话  | 角色立场、边界、判断、风格、视觉指令和最终措辞             | `src/kazusa_ai_chatbot/nodes/`                                        |
