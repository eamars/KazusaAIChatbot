...
| 日历式后续行动   | 被角色接受的未来承诺和到期约定可以转成持久日历触发，稍后重新进入新一轮认知。        |
...
| 承诺与后续行动 | 已接受的未来事项会经过校验、去重、持久化和日历触发，而不是靠模型临场记住。        |
...
聊天 LLM 调用统一通过 `LLInterface`。各模块继续拥有自己的路由、模型、生成预算和 thinking 开关配置；接口层负责后端识别、provider 会话、请求映射、响应归一化和模型卸载重试。运行时合约见
...
被选中的可见文本表面通过 `ChatResponse` 返回给适配器，并由发送回执补全平台消息 id。私有动作结果、无可见输出的判断和表面轨迹，仍然可以进入回合后的对话进度、整合、Cache2 失效、私念残留、日历状态、反思和自我认知流程，但不会自动变成平台发送。

延迟用户任务会先表现为角色已经接受的任务。L2d 只看到语义化的 `accepted_task_request` 和 `accepted_task_status_check`；确定性动作执行在去重和生命周期持久化之后，才把新任务物化为内部 `background_work` 执行。完成后的任务以 `accepted_task_result_ready` 重新进入认知，而不是由 worker 直接发送平台消息。
...
Kazusa 不把所有上下文塞进一个超长提示词。最近文本、短期话题进度、检索证据、长期记忆、已提升反思、日历触发的承诺，各自有独立生命周期。

私念残留是单独的短期通道。它只保存已完成回合里仍可能影响下一次理解的一条第一人称私人原因，并且只作为 `internal_monologue_residue_context` 进入 L2a。它不是 `reflection_summary`，不是长期记忆，不是可见对话计划，也不是日历输入。
...
适配器负责平台语法和投递。角色身份、记忆、检索、认知、日历调度都属于平台无关核心。
...
| 控制台    | 本地操作者认证、服务生命周期、日志、审计、静态 UI 和调试聊天衔接 | [控制台接口文档](src/control_console/README.md) |
...
| LLM 接口 | 聊天模型调用兼容层、provider 会话、路由诊断和模型卸载重试 | [LLM 接口文档](src/kazusa_ai_chatbot/llm_interface/README.md) |
...
| 认知解析器  | 有上限的循环状态、能力观察、人工澄清/恢复和解析轨迹            | [认知解析器接口文档](src/kazusa_ai_chatbot/cognition_resolver/README.md)    |
...
| 已接受任务  | 角色已经接受的延迟用户任务生命周期                    | [已接受任务接口文档](src/kazusa_ai_chatbot/accepted_task/README.md) |
| 后台工作   | 内部延迟任务执行、worker 路由和结果回传                | [后台工作接口文档](src/kazusa_ai_chatbot/background_work/README.md) |
...
| 日历调度器 | 未来认知、到期约定和反思阶段槽位的持久类型化触发            | [日历调度器接口文档](src/kazusa_ai_chatbot/calendar_scheduler/README.md) |
| 分发器    | 面向适配器的投递校验和回调传输辅助                         | [分发器](src/kazusa_ai_chatbot/dispatcher/README.md)                     |
...
正常本地运行先启动 buildless Python/FastAPI 控制台，再通过控制台启动或停止脑服务和适配器：

```powershell
kazusa-control-console --host 127.0.0.1 --port 8765
```

只有在绕过控制台做开发调试时，才直接运行脑服务：
...
src/
  ...
  kazusa_ai_chatbot/
    ...
    calendar_scheduler/        持久类型化触发调度和迁移脚本支持
    dispatcher/                面向适配器的投递校验和衔接
...
Kazusa 认知核心仍处于 alpha 阶段。当前主运行时已经可以作为本地脑服务使用，具备平台适配器、记忆、检索、自我认知、反思和日历调度能力；部分自主联系能力仍停留在带权限的预览合约中，还不是生产发送路径。
...
| [文档规范](docs/DOCUMENTATION_GUIDE.md)                           | 文档角色、事实来源层级、模块 README 规则和中英文一致性 |
| [子智能体接口指南](docs/SUBAGENT_INTERFACES.md)                    | 子智能体和 worker 家族的统一文档词汇 |
| [控制台接口文档](src/control_console/README.md)                       | 本地控制台、认证、服务生命周期、日志和静态 UI |
...
| [LLM 接口文档](src/kazusa_ai_chatbot/llm_interface/README.md)       | 聊天模型调用、后端兼容和路由诊断 |
...
| [已接受任务接口文档](src/kazusa_ai_chatbot/accepted_task/README.md)       | 用户可见的延迟任务生命周期        |
| [后台工作接口文档](src/kazusa_ai_chatbot/background_work/README.md)       | 内部延迟任务执行器和 worker 合约 |
...
| [认知解析器接口文档](src/kazusa_ai_chatbot/cognition_resolver/README.md) | 有上限的解析循环、能力观察、人工澄清和轨迹 |
