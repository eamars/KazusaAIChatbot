...
| 内部独白残留 | 已完成回合会留下很短的第一人称私人原因，只投给下一次 L2a 认知。 |
...
| 已接受的延迟任务 | 已接受的提醒和文本任务会被持久化、路由给内部后台 worker，并通过认知返回，而不是由 worker 直接发送。 |
...
flowchart TD
    ...
    subgraph Maintenance["后台和持久子系统"]
        ...
        CONS["consolidation<br/>目标计划 -> 抽取 -> 写入意图校验 -> 持久化"]
...
会在实时回合之后选择 worker 和语义任务；text-artifact worker 有自己的任务
router 和 generator。已完成的 accepted task 会作为 `accepted_task_result_ready`
认知返回，而不是由 worker 直接发送。旧的 background-artifact 和旧的
background-work 行仍然只是兼容数据，不是新的面向模型的运行时合约。
...
内部独白残留通道是一个单独的短期通道。它保存已完成回合中的一条紧凑第一人称原因，
...
| 内部独白残留 | 只加载到 L2a 认知的短期私人第一人称残留 | [内部独白残留 ICD](src/kazusa_ai_chatbot/internal_monologue_residue/README.md) |
...
| 整合 | 持久目标路由、写入意图校验和按目标分离的持久化 | [整合 ICD](src/kazusa_ai_chatbot/consolidation/README.md) |
...
src/
  ...
  kazusa_ai_chatbot/
    ...
    consolidation/             持久整合 helper、目标路由和 ICD
    rag/                       RAG 2 辅助智能体、混合检索、Cache2
    conversation_progress/     短期回合记忆
    internal_monologue_residue/ L2a 使用的内部独白残留通道
