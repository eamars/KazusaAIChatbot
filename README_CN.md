...
角色回合
  ...
  - 只为 L2a 读取投影后的私念残留
  - 进入有上限的认知解析器
  - 每一轮解析都完整经过 L1 -> L2 -> L2d
  - 只有当认知选择需要时，才调用 RAG 2、网页/实时证据、人工澄清、审批准备或私有自我解析能力
  - 运行被选中的 L3 文本/动作表面
  - 输出可见表面或私有动作结果
        |
        +-----------------------------> 适配器发送可见回复
...
```text
平台适配器 / 调试客户端 -> 脑服务 -> 队列与入口处理 -> 类型化回合
-> 认知解析器 / L2d -> 按需证据能力 -> L3 表面与动作
-> 回合证据整合 -> 调度 / 反思
```
...
RAG 2 回答“现在知道什么”，但它不再是每个角色回合的前置步骤。认知解析器先判断当前目标和缺口；只有认知需要证据时，才把 RAG 2 当作能力调用。认知系统回答“这些信息对当前角色意味着什么”，对话系统回答“角色应该怎样说出来”。
