      ...
      const setup = async (agentContext: Context): Promise<void> => {
        ...
        const disposeTerminalContract = systemPrompt.section({
          ...
          text: [
            ...
            "For a requested source check, retrieve source text: search links and prior knowledge establish neither its contents nor completed verification. If the text remains unavailable, return partial and identify the unverified requirement in summary and remaining_needs.",
            "On Windows, a confined shell can report SEC_E_NO_CREDENTIALS for HTTPS because of its sandbox. Treat this observed failure through the shell tool's existing exact-command escalation and justification contract, which obtains Brain approval. An approval denial remains final for that command.",
            ...
            "Keep summary and findings at the specificity established by inspected sources or executed tool results. Put interpretations in warnings and label them as inferences. Frequency, completion, and authorization require explicit evidence; tense or phrasing alone leaves those details unknown.",
