...
| `tone`                                      | Broad voice setting. Dead right now, suggest to remove | Static unless a separate voice calibration plan approves change.                                                                                                         |
| `speech_patterns`                           | Canonical speech style. Dead right now, suggest to remove                          | Static unless a separate voice calibration plan approves change.                                                                                                         |
...
## Stable Profile Versus Memory Lore

The deeper duplication scan found exact counterparts for the stable profile
fields in both the local memory source and the active runtime `memory`
collection. These rows are seeded shared-memory facts whose confidence notes
state that they were copied from `personalities/kazusa.json`.

Architectural direction: `character_state` and the public character-profile
projection are the canonical source for compact, stable active-character
identity facts. The shared `memory` collection should not carry one-to-one
copies of those same facts. Memory should carry expanded world lore,
...
| `gender`             | `character_state.gender` | `杏山千纱-性别` | Remove or deprecate the memory duplicate. This is a static identity fact and should stay profile-owned. |
| `age`                | `character_state.age` | `杏山千纱-年龄` | Remove or deprecate the memory duplicate. This is a static identity fact and should stay profile-owned. |
| `birthday`           | `character_state.birthday` | `杏山千纱-生日` | Remove or deprecate the memory duplicate. The profile already carries the date and zodiac-formatted value. |
| `backstory`          | `character_state.backstory` | `杏山千纱-背景故事` | Remove or deprecate the exact memory duplicate. Keep specific lore rows for sub-events, aliases, organizations, relationships, and setting context. |

The inverse ownership choice is weaker: removing these fields from the profile
and relying on memory would make self-introduction and profile questions depend
on retrieval quality, ranking, and whatever other world-lore rows happen to
match. The current RAG routing already has a first-class active-character
profile path for self-description and self-introduction, so the stable profile
should remain the source of truth.

Rows such as `杏山千纱-基础身份` and `杏山千纱-黑历史` overlap with the profile
but are not automatically duplicate if they provide source-backed aliases,
setting context, or a focused sub-event. They should be reviewed by content
role, not deleted just because they mention age, school, or the past. The
questionable seeded row `杏山千纱-官方住址` is not a profile duplicate; it should
be reviewed separately because it appears to introduce a high-authority
"official" fact outside the stable profile.

Reflection-promoted rows can also overlap with profile facts. Examples found
include `杏山千纱 (Kyōyama Kazusa) 中学化名“凯茜·帕鲁格”`, `化名往事`, and
`甜点部身份与强烈反应`. These should not become canonical profile sources. A
later memory cleanup should either keep only the unique behavioral or episode
signal, merge true lore detail into a curated lore row, or remove the row when
it merely restates stable profile/backstory facts.
