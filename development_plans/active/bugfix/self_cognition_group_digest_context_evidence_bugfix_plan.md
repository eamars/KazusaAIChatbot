...
- Acceptance criteria: the plan implements and tests all four requested parts:
  ...
  conversation evidence from `group_digest_summary`, and real LLM test of that
  evidence value.
...
2. Add a separate string-only `group_digest_summary`, intentionally short,
   describing general topics like relevance's `channel_topic`.
3. Feed `group_digest_summary` to conversation search to retrieve possible
...
- `group_digest_summary` is one intentionally short string. It describes the
...
- `conversation_progress.group_scene_digest` must keep the existing shape
  `{"digest": str}`. Store `group_digest_summary` as a separate optional
  string field, not nested inside `group_scene_digest`.
...
- Add `group_digest_summary` as a separate short string generated by the group
  digest module without changing the existing `group_scene_digest.digest`
  contract.
- Use `group_digest_summary` as the plain-string task seed for pre-cognition
...
| `group_digest_summary` | compatible | Add one optional string-only source-hydration field. Omit it when unavailable. |
...
selected 15-minute group window
  ...
  -> short group_digest_summary string
...
`group_digest_summary` are strings. Conversation evidence enters as existing
...
| Summary output | Add one short string `group_digest_summary` | Mirrors relevance `channel_topic` style without introducing structured LLM-to-LLM payloads. |
...
conversation_progress["group_digest_summary"] = str
```

Rules:

- `digest` is the existing observational digest string.
- `group_digest_summary` is a short topic string.
...
explain this group topic: <group_digest_summary>
```
...
- Stage 1 adds no new source type. It widens the rows passed into the existing
  group digest path.
- Stage 1 adds one short `group_digest_summary` string. It may be generated in
...
- Stage 3 adds one bounded pre-cognition conversation evidence call for
  group-review cases where `group_digest_summary` is non-empty.
...
- `src/kazusa_ai_chatbot/reflection_cycle/group_scene_digest.py`
  ...
  - Add one short string output named `group_digest_summary`.
...
- `src/kazusa_ai_chatbot/self_cognition/sources.py`
  ...
  - Attach `conversation_progress.group_digest_summary = str` when available.
...
```powershell
venv\Scripts\python.exe -m pytest tests/test_self_cognition_response_sensitivity_live_llm.py::test_live_self_cognition_laxi_ambient_group_digest_does_not_speak -q -s -m live_llm
```

Expected baseline: under production backend, the test fails by selecting
visible speech. This nonzero pytest exit is expected baseline evidence, not a
completed fix gate.
...
3. Add one separate short `group_digest_summary` string output and validation.
4. Keep `group_digest_summary` string-only and intentionally short.
...
- If Stage 1 no longer selects visible speech and L2a no longer rewrites W's
  `你` as Kazusa, record that expanded digest may be enough.
...
3. Write a human-authored review artifact comparing Stage 1 vs Stage 3:
   - `group_digest_summary` used as retrieval seed,
...
- [ ] Stage 0 - baseline reproduction evidence recorded.
  - Covers: current failing behavior under production backend.
  - Verify: `baseline_15m_digest` trace selects visible speech; the nonzero
    pytest exit is recorded as expected baseline evidence.
  - Evidence: trace path and human-authored review artifact.
  - Sign-off: record agent/date and exact command output summary.

- [ ] Stage 1 - expanded digest input and summary implemented.
...
- [ ] Stage 2 - expanded digest real LLM evidence recorded.
  ...
  - Sign-off: record agent/date and Stage 1 quality judgment.

- [ ] Stage 3 - conversation evidence from summary implemented.
...
- [ ] Stage 4 - conversation evidence real LLM value recorded.
...
- [ ] Independent Code Review gate completed.
  ...
  - Sign-off: record agent/date and final status.
