# coding agent phase2 code writing plan superseded 2026-06-23
...
- Reason: replaced by a new Phase 2 plan centered on feature-level interface
  contracts, role-level live LLM suites generated from every hard gate, and an
  explicit no-E2E-before-role-evidence rule.
- Execution rule: do not execute this archived plan. Use the active
  replacement plan instead.
...
- The top-level coding supervisor is the only cross-domain dispatcher for
  Phase 2. It owns the fetch/read/write/external loop, durable run ledger,
  public evidence aggregation, and final response assembly. Code subagents
  must not privately call peer code subagents.
...
- The code-writing supervisor owns local writing repair. Validator failures,
  patcher blocked diagnostics, programmer contract failures, Module PM repair,
  file-agent repair, and writing-PM repair stay inside `code_writing` until the
  local repair limit is reached, the writing workflow needs source reading or
  external evidence, or a validated proposal is ready.
...
- PM stages are layered. The top-level writing PM owns semantic decomposition,
  ...
  file-to-file input/output contracts, cross-file imports, Module PM review
  ...
  paths. One Module PM runs for each accepted module/file assignment and owns
...
- The file agent must run after source ownership and before the file plan
  evaluator, Module PMs, or programmer calls. It validates writing-PM file
...
- The file plan evaluator must run after the file agent packages a writing PM
  decision and before any Module PM call. It validates the resolved file/module
  ownership contract and returns compact repair feedback to the top-level
  writing PM inside a bounded repair loop.
- The module contract evaluator must run after a Module PM emits one
  ...
  feedback to the Module PM inside a bounded repair loop.
- Code-reading and code-writing PM stages enforce interfaces and coordinate
  ...
  programmer evidence. In writing, the top-level writing PM and Module PMs must
  ...
  owner paths from evidence before file mechanics run. Module PMs describe
...
- A dedicated writing patcher owns patch materialization after the top-level
  writing PM selects reviewed programmer outputs. The patcher converts
  writing-PM-selected implementation content,
...
- The code-writing supervisor and deterministic validation must reject
  programmer reports
  outside assigned scope and repair loops that broaden a programmer assignment
  without a new Module PM contract accepted by the module contract evaluator.
  The file plan evaluator owns pre-Module-PM rejection for overlapping file
...
- Support multi-file composition through PM/programmer writing reports and
  writing-PM-owned reconciliation. Do not ask one LLM prompt to write a repository-wide
  patch from raw files.
- Split the current broad writing path into a top-level writing PM plus one
  Module PM per accepted module/file assignment. The top-level writing PM
  manages whole-request decomposition, file/module ownership needs,
  file-to-file input/output contracts, cross-file imports, and Module PM review
  reconciliation. Each Module PM manages one module-level programmer contract
  for its assigned file/module.
- Add a dedicated writing patcher role after PM/programmer reconciliation. The
  patcher materializes writing-PM-selected programmer content into unified diffs or
...
- Clean up the current Phase 2 code toward the role split before further gate
  ...
  agent -> file plan evaluator -> Module PM -> module contract evaluator ->
...
- Enforce the layered PM-to-programmer interface contract: top-level writing
  ...
  scope; each Module PM defines one module-level programmer contract with exact
...
- Add focused pre-E2E real LLM role gates for the reading PM, reading
  programmer, top-level writing PM, Module PM, module programmer, patcher,
...
-> Module PMs define module-level programmer contracts
-> module contract evaluator accepts module-level contracts
-> bounded writing programmer workers
-> writing patcher materializes writing-PM-selected programmer output
...
|---|---|---|
| Writing architecture | Use a top-level writing PM, one Module PM per accepted module/file assignment, module-level programmers, and a patcher after programmer reports | Multi-file patching needs local-LLM context partitioning across whole-request coordination, file ownership, module contracts, implementation, and edit mechanics so no local model call owns too much of the feature. |
...
| File planning | Shared file agent validates accepted owner paths, reserves new files, and packages file contracts before file plan evaluation and Module PM dispatch | Keeps path safety, permission, collision, source-scope, current file context, and mutex management out of programmer prompts while giving Module PMs, validators, and the patcher the file metadata they need. |
| Patch materialization | Dedicated patcher materializes writing-PM-selected programmer output into unified diffs or new-project file trees | Keeps semantic implementation, edit mechanics, and structural validation in separate roles. |
...
|---|---|---|
| Top-level coding supervisor | Owns the public request, fetch/read/write/external loop, context ledger, retry budget, and final public response assembly. | Supervisor trace shows each selected workflow action, evidence handoff, loop count, and final status. |
...
| Top-level writing PM | Selects writing mode, defines semantic file demands, file/module ownership needs, file-to-file inputs/outputs, cross-file imports, Module PM responsibilities, report reconciliation, and patcher input packet. | Writing-PM trace shows mode decision, file demands, file/module contracts, cross-file imports, selected module artifacts, and unresolved gaps. |
...
| File plan evaluator | Runs deterministic structural checks on file-agent-packaged file/module contracts before Module PM dispatch and sends compact repair feedback to the top-level writing PM. | Evaluator trace shows accepted or repair-required status, file/module contract ids, bounded error list, and repair attempt count. |
| Module PM | Defines one module-level programmer contract for one accepted file/module assignment and reviews the programmer output for that assignment. | Module-PM trace shows file label, edit mode, imports, current file context summary, symbols to define, required behavior, review status, and unresolved gaps. |
| Module contract evaluator | Runs deterministic structural checks on module-level programmer contracts before programmer dispatch and sends compact repair feedback to the Module PM. | Evaluator trace shows accepted or repair-required status, file label, required keys, prompt budget, leaked file-management fields, and repair attempt count. |
...
| Writing patcher | Converts writing-PM-selected programmer content into unified diffs or new-project file-tree artifacts and reports edit diagnostics. | Patcher trace shows input packet id, owned path map, changed files, artifacts, diagnostics, and blocked status when material is insufficient. |
...
flowchart TD
    ...
    FPE -->|accepted file/module contract| FPM1[Module PM]
    FPE -->|accepted file/module contract| FPM2[Module PM]
    ...
    P -->|patch artifacts or blocked diagnostics| V[Deterministic validator]
    V -->|validation feedback or approval| WPM
    WPM -->|validated proposal or terminal writing status| S
...
### Writing Module PM Input

```python
WritingModulePMInput = {
    "question": str,
    "mode": str,
...
`status="need_module_pms"` means the top-level writing PM has enough evidence
...
concrete file/module contracts before the file plan evaluator and Module PMs run.
...
writing-module-PM repair call with `file_resolution_feedback` and no file plan
evaluator, Module PM, or programmer call is consumed. If the repaired PM
...
The supervisor evaluates each file-agent-packaged `need_module_pms` PM decision
before Module PM dispatch. The evaluator checks only
...
writing-PM repair call with the evaluator feedback and no Module PM or
programmer call is consumed. If the repaired writing-PM decision still does not
...
The Module PM sees one file/module contract and selected evidence for that
...
### Module PM Programmer Contract

```python
ModulePMProgrammerContract = {
...
The supervisor evaluates each Module PM programmer contract before programmer
...
If evaluation returns `repair_required`, the supervisor makes one bounded
Module PM repair call with the evaluator feedback and no programmer call is
...
### Module PM Review Report

```python
ModulePMReviewReport = {
    "file_contract_id": str,
    "file_label": str,
...
- Writing session memory:
  - stores base identity, top-level writing PM decisions, file/module
    contracts, Module PM programmer contracts, programmer reports, Module PM
...
- Module PM memory:
  ...
  - returns one module-level programmer contract or a compact Module PM review.
...
| Top-level writing PM planning/sufficiency | 30k tokens | 8k tokens | User goal, repo/source summary, compact reading facts, session summary, selected external evidence, Module PM review summaries, validation summaries. |
| Module PM planning/review | 24k tokens | 6k tokens | One file/module contract, selected file evidence, current file context, imports, previous module report for that file/module, evaluator feedback. |
...
| PM final selection/synthesis | 34k tokens | 8k tokens | User goal, compact Module PM review set, selected patch fragments, validation summaries, public-response constraints. |
| External evidence request prompt | 16k tokens | 4k tokens | Module-PM-authored evidence need, public source hints, bounded repository summary. |
...
- Target cap: keep ordinary top-level writing PM inputs at or below 30k tokens,
  ordinary Module PM inputs at or below 24k tokens, and ordinary
...
- Trace memory: live LLM reviews record prompt estimates, selected evidence
  ids, pruned evidence counts, PM decisions, Module PM decisions, programmer
...
- If one module-level programmer contract exceeds budget, the Module PM narrows
...
- No standalone top-level writing PM, Module PM, writing programmer, external
...
- Existing-repository patch path:
  ...
  - Up to five Module PM calls per wave on `CODING_AGENT_PM_LLM`.
...
- `src/kazusa_ai_chatbot/coding_agent/code_writing/file_product_manager.py`:
  LLM Module PM prompt, module-level programmer contract generation, and Module PM
  review parsing.
...
- `src/kazusa_ai_chatbot/coding_agent/code_writing/patcher.py`: patch
  materialization prompt and report parsing; converts writing-PM-selected programmer
...
- `src/kazusa_ai_chatbot/coding_agent/code_writing/supervisor.py`: route
  ...
  file/module contracts through Module PMs, route accepted module contracts to
...
- `src/kazusa_ai_chatbot/coding_agent/code_writing/models.py`: align writing
  PM, Module PM, module programmer, patcher, validation, and result data shapes
...
- `src/kazusa_ai_chatbot/coding_agent/code_writing/module_product_manager.py`:
  ...
  active runtime code.
- `src/kazusa_ai_chatbot/coding_agent/code_writing/module_contract_evaluator.py`:
  keep as the active structural evaluator for Module-PM-authored module
...
- `tests/test_coding_agent_writing.py`,
  ...
  writing PM selection, Module PM module contracts, module programmer reports,
  Module PM reviews, patcher materialization, validation, and synthesis in that
  order.
...
- Ownership boundaries: the top-level writing PM owns semantic decomposition,
  ...
  sufficiency, and selected artifact packets; Module PMs own one module-level
...
7. Executing agent cleans up any current Phase 2 code that predates the
   ...
   - Route programmer reports through a writing-PM-selected
     `WritingPatcherInput`
   ...
   - Update tests and traces so a succeeded proposal without a patcher report
     fails.
8. Executing agent implements top-level writing PM, Source Ownership PM,
   Module PM, module programmer, patcher, and synthesis.
   ...
   - Add Source Ownership PM prompt/handler using `CODING_AGENT_PM_LLM` to
     ...
     candidate paths.
   - Add Module PM prompt/handler using `CODING_AGENT_PM_LLM`.
   - Add the shared file agent and route writing-PM semantic file demands
     ...
     evaluation and Module PM dispatch.
   ...
   - Add file-agent resolution before Module PM dispatch. It validates safe
    ...
    - Add the file plan evaluator after file-agent resolution and before Module
   ...
   - Add the module contract evaluator after Module PM contract generation and
     ...
     provides one bounded Module PM repair loop.
   ...
   - Add deterministic validation that patcher output stays inside the
    ...
    - Add report compaction and writing-PM final artifact selection. PM
      ...
      test bodies.
   - Integrate the local writing repair loop in this order:
     - keep `need_reading` and `need_external_evidence` as the only
       cross-domain outcomes returned to the top-level coding supervisor;
     - keep file-agent, file-plan, and module-contract repair inside the
       existing bounded repair loops;
     - after patcher materialization, route validator failure summaries back to
       the top-level writing PM with the current programmer reports and patcher
       diagnostics;
     - the writing PM may narrow module contracts, request another Module PM
       contract, rerun one programmer, or ask the patcher to rematerialize
       selected artifacts, but it may not write code itself;
     - return `failed` or `rejected` to the top-level supervisor only after the
       local writing repair budget is exhausted or the request is unsupported;
     - record each local repair attempt in the writing trace before any E2E gate
       can be considered ready.
     The patcher may materialize selected fragments into diff hunks, but it
     must not add feature behavior that was not present in writing-PM-selected
     programmer content.
...
12. Parent runs focused pre-E2E real LLM role gates one at a time.
    - Run the reading PM, reading programmer, top-level writing PM, Module PM,
...
- Runtime `code_writing` uses layered PM/programmer LLM roles behind the public
  ...
  file-to-file contracts, cross-file imports, and artifact selection. Module PMs
...
- Runtime `code_writing` uses a patcher after writing-PM-selected
  programmer reports.
...
- [ ] Stage 3 - role-boundary cleanup and module-level PM/programmer/patcher writing
  ...
  - Verify: patched LLM writing-PM/Module-PM/programmer/patcher contract tests pass,
    ...
    file plan evaluator repair feedback before Module PM dispatch,
    Module-PM-owned module contracts, module contract evaluator repair feedback
    ...
    tests prove writing-PM/Module-PM/programmer/patcher prompt inputs stay within
...
- Phase 2 hard gates prove artifact-quality writing behavior. A passing gate
  means the top-level supervisor followed the intended workflow, the top-level
  writing PM decomposed the request into file/module contracts and semantic file
  demands, the file agent resolved concrete owned paths, the file plan
  evaluator accepted file/module contracts, Module PMs produced module-level
  ...
  content, Module PMs reviewed those outputs, the patcher materialized selected
...
- Environment failure, missing model route, missing explicit workspace,
  ...
  writing-PM-selected paths is a hard-gate failure.
...
| `role_05` | Module PM | One accepted `WritingFileModuleContract`, selected evidence, current file context, prior module report for the same file/module, and evaluator feedback. | `ModulePMProgrammerContract` with edit mode, exact imports, bounded current file context, `symbols_to_define`, and required behavior. | Defines one module-level programmer contract, does not include path/mutex/base-revision fields in programmer input, and does not reference peer programmer output as required implementation context. |
| `role_06` | Module programmer | One accepted `ModulePMProgrammerContract` with exact imports, current file context, symbols to define, and required behavior. | `WritingProgrammerReport` with one fenced code artifact for that module contract. | Implements only the assigned module contract, preserves the imports and symbol contract, returns one code artifact, records risks/open questions, and does not change peer interfaces. |
...
- Add and run focused tests that prove:
  ...
  - a successful writing response requires a patcher report between writing
    module PM selection and patch validation;
  ...
  - patcher reports are rejected when they materialize files outside the
    writing-PM-selected owned path map;
...
- The top-level writing PM defines semantic file demands, file/module
  ...
  deliverables, and validation expectations for each Module PM assignment.
...
- The file agent validates accepted owner paths and placement data into
  ...
  mutex ids before the file plan evaluator and Module PMs.
- The file plan evaluator accepts file/module contracts before Module PM
  dispatch.
- Module PMs define explicit module-level programmer contracts with edit mode,
...
- Module PM reviews accept programmer output before patcher materialization.
...
- PM/programmer writing architecture mirrors Phase 1 ownership, adds a
  top-level writing PM plus Module PMs, and keeps
...
- PM stages enforce interface contracts and coordinate programmer outputs for
  ...
  evidence. The top-level writing PM and Module PMs do not author implementation
  ...
  evaluation and Module PM dispatch. Module PMs express module-level programmer contracts before module contract
...
- Multi-file patch composition is supported through bounded programmer
  reports, Module PM reviews, writing-PM reconciliation, patcher
...
- Supervisor, reading PM, top-level writing PM, Module PM, programmer,
...
| Multi-file patch loses interface consistency | Writing-PM reconciliation over Module PM reviews plus patcher materialization checks | Multi-file deterministic acceptance and hard artifact gates |
...
| New-project mode becomes a canned template engine | Writing-PM/Module-PM/programmer generation plus anti-cheat and external evidence | New-project hard gate and static grep |
...
| Context overflow degrades local LLM quality | Supervisor-owned run ledger, private evidence store, compact PM/Module-PM/programmer reports, hard prompt-build caps | Context-budget tests and live trace review |
...
- User clarification: the function-level control path moved too much
  ...
  context. Phase 2 writing now uses one top-level writing PM, one Module PM per
...
- Architecture reference update:
  ...
  -> file plan evaluator -> Module PM -> module contract evaluator ->
  programmer -> Module PM review -> patcher -> validator -> synthesizer.
- IO contract update:
...
- Plan update:
  ...
  File Agent, File Plan Evaluator, Module PM, Module Contract Evaluator, Module
  Programmer, Module PM Review, Patcher, Structural Validator, File Level
  Validator, and Synthesizer.
- Cleanup direction:
  current production code and tests must be migrated from the previous
  intermediate coordinator/evaluator path to the module-level Module PM path
...
- Production change:
  ...
  `module_product_manager.py` now exposes a module-level Module PM
...
- Agent-authored review:
  ...
  decomposed the gate-02 task too generically, and several Module PM contracts
...
- Production cleanup:
  ...
  `need_module_pms`, removed the stale function/unit programmer active code
  paths, and made the active supervisor run the current sequence:
  writing PM -> File Agent -> file plan evaluator -> Module PM -> module
...
- Production wiring:
  ...
  contracts. The writing supervisor now passes those imports into the Module PM
...
- Contract alignment:
  ...
  `need_module_pms` status and parser shape. The contract records that
