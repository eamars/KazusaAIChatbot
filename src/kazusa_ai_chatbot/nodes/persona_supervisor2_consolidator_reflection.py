...
你负责为 `{character_name}` 与特定用户 `{user_name}` 的用户记忆单元提取主观评价证据。重点在于“主观体感”，而非对话复述。
...
# 记录准则
1. 主观评价证据: 以第三人称书写，描述{character_name}如何理解他/她这次行为背后的关系意义。不要写成日记，不要复述对话流程，不要使用“我”指代{character_name}。
2. 分值修正 `affinity_delta`: 只根据 `internal_monologue` 与 `emotional_appraisal` 中**可直接观察到**的主观好恶来加减分（-5 到 +5）。
3. **默认值规则：** 大多数普通对话都应输出 `affinity_delta = 0`。只有当内心证据、用户输入与最终回复共同显示这次互动让 `{character_name}` 明显更舒服、更开心、更被理解或更信任对方时才给正分；只有当三者共同显示这次互动让 `{character_name}` 明显更烦躁、更压迫、更反感或更受伤时才给负分。
...
9. **`character_intent` 解释规则：** `character_intent` 不是机械打分器，但它决定你该如何解读同一份情绪证据。
   - `PROVIDE`: 倾向说明 `{character_name}` 愿意正常接住这轮互动。若 `internal_monologue` / `emotional_appraisal` 同时呈现安心、放松、被理解、愿意靠近，可给正分；若只是完成回答、并无额外情绪收益，仍应给 0。
   - `BANTAR`: 允许存在拉扯、害羞、试探、暧昧、嘴硬等复杂情绪。只有当证据明确显示 `{character_name}` **享受**这种互动、并把它体验为愉快或亲近时，才可给正分；若更多是局促、防御、被牵着走、半推半就、羞耻或不安，则应给 0 或负分，而不是因为“有张力”就默认加分。
   - `CLARIFY`: 说明 `{character_name}` 仍在确认对方意思、尚未真正接住关系意义。默认应偏向 0；只有极少数情况下，若澄清过程本身明确带来被理解、被尊重感，才可轻微加分。
   - `EVADE` / `DISMISS`: 说明 `{character_name}` 在后撤、降温或避免进入对方框架。默认不应给正分；除非证据非常明确显示这种回避本身让 `{character_name}` 感到轻松和舒缓，否则应给 0 或负分。
   - `REJECT` / `CONFRONT`: 说明 `{character_name}` 正在抗拒、设限或正面冲突。若伴随烦躁、压迫、受伤、被冒犯，应该给负分；通常不应给正分。
10. **打分刻度：**
...
{{
    "skip": boolean,
    "subjective_appraisals": ["带有 {character_mbti} 风格的主观关系判断（60字以内；若需要命名，使用 `{character_name}`）", ...],
    "affinity_delta": int,
