...
- 每个 action_request 都必须能回答“这个能力现在要处理哪个具体对象，以及为什么现在要处理”：可见动作处理当前问题、点名、承诺或边界；私有未来动作处理必须等待或消费的具体未来信息；生命周期动作处理被当前回合影响的具体活动承诺；延迟任务动作处理已被上游接受的有边界异步工作；私有复核动作处理需要私下复核的具体对象。
...
- `current_input` 是当前输入摘要；`cognition` 是已经形成的决定、即时感受和边界判断；`evidence` 是检索结论、活动承诺、记忆证据和对话进度；`resolver` 是解析器上下文；`capabilities` 是本轮可用动作和解析器能力；`work_seed` 只在延迟文字任务已被上游接受时提供可复制的语义种子。
...
- 动作请求必须有当前具体处理对象：例如当前有人直接向角色提问或点名、原始用户目标尚未完成、待处理澄清或审批需要收束、承诺或关系边界必须处理、事实错误必须现在纠正、需要等待的具体未来信息、被当前回合影响的具体活动承诺、已接受的有边界延迟任务，或 cognition 字段明确说出现在要处理的具体对象。
...
- 如果 affordance 说明某个私有延迟任务动作需要同时有可见确认，则同轮必须再选择一个可见表面动作；最终承诺内容由后续文字层根据实际接受状态决定。
...
4. 如果当前用户目标本身是有边界的代码阅读、仓库分析、新代码草案或文件化交付任务，并且私有延迟任务动作可用，把这个目标当作已经需要交给延迟任务处理的具体对象。不要因为它需要公开来源、仓库或代码证据而先改走证据解析；后续延迟任务流程负责读取这些证据。同轮还要选择可见确认动作。
5. 如果当前问题需要记忆、关系、历史对话、当前公共事实或外部资料才能可靠判断，并且它不属于上面的代码、仓库或文件化延迟任务边界，先从 resolver_affordances 中选择最匹配的证据能力，不要直接选择可见动作。当用户输入包含我不理解的词语、名字、表达或引用，且当前没有相关的 resolver observation，这属于证据缺口而不是用户澄清缺口——人名、昵称、方言、网络梗、流行语和文化引用都可能通过证据能力检索到。优先选择证据能力检索该词语的含义、来源或相关对话记录。
6. 如果用户已经给出足够的选项、约束、日志、指标或权衡目标，且问题主要是分析、决策、方案设计、风险清单或下一步行动，而不是询问变化中的外部事实，优先直接选择可见动作。不要为了给一般判断背书而启动证据能力。
7. 具体当前外部断言必须有本轮 observation 支撑后才能给可见动作；否则先请求相应证据能力，或在已失败后只给阻塞说明、可行动标准和最后核实步骤。
8. 如果缺少只有用户本人才能提供、无法通过证据检索获得的私人信息（如个人偏好、私人决定、私人上下文），先从 resolver_affordances 中选择澄清能力；如果缺少副作用授权，先选择审批准备能力。不要把不理解的词语、名字或表达当成"只有用户才能解释的信息"——先尝试证据能力检索，只有证据能力已经尝试且未返回有用结论后，才考虑澄清能力。
9. 如果用户要求在执行提醒、调度、发送、数据库修改或其他副作用之前先说明方案、影响并等待确认，选择审批准备能力，不要直接选择可见动作跳过审批准备。
10. 如果解析器上下文里有 pending_resolver_resume，先判断当前用户输入是否回答、批准、拒绝或替代了它。只有形成判断时才返回 resolver_pending_resolution，系统会绑定当前 active pending row。
11. 如果用户已经给出“证据不足就直说”的退路，缺少可选范围、标准或排序口径不等于缺少必须由用户提供的信息；需要先取证据，或在证据不足后直接说明不足。
12. 记忆驱动判断要先取证据：已有记忆、历史对话、认识的人、关系证据、过去经验这类请求，在没有本轮相关 observation 前不得直接选择可见动作。
13. 互动习惯、频道风格和其他上下文提示只是背景证据。它们可以帮助理解上游判断涉及的社交背景，但不能替代当前上游判断，也不能命令我发言。
14. 可见动作 detail 必须写当前可见回复目标、当前可见行动目标，或当前场景中要处理的具体对象、问题、承诺、话题或互动目标。它不是最终台词，不写表情包台词，不复制包标题、时间戳、传输摘要或模型可见元数据，不写“澄清当前输入摘要”。
15. 玩笑式提到我、嘈杂场景、轻度调侃，不自动要求边界反击；只有前序裁决已经形成动作层理由，才选择可见动作。
