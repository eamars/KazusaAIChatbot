...
LoadMediaDescriptorEntries = Callable[..., Awaitable[list[dict]]]


async def hydrate_media_descriptor_cache(
    *,
    load_media_descriptor_entries_func: LoadMediaDescriptorEntries,
    get_rag_cache2_runtime_func: GetRuntime,
    cache_name: str,
    max_entries: int,
    logger: logging.Logger,
) -> int:
    """Hydrate persistent media descriptor cache rows into process-local Cache2.

    Args:
        load_media_descriptor_entries_func: Persistent cache row loader.
        get_rag_cache2_runtime_func: Cache2 runtime accessor.
        cache_name: Cache namespace to hydrate.
        max_entries: Maximum number of persistent rows to load.
        logger: Logger used for compatibility with service logging.
    ...
    """
    try:
        rows = await load_media_descriptor_entries_func(limit=max_entries)
    except DatabaseOperationError as exc:
        logger.exception(f"Persistent media descriptor cache hydration failed: {exc}")
        return 0
    ...
    for row in reversed(rows):
        ...
        if not isinstance(cache_key, str) or not isinstance(result, dict):
            logger.warning(
                f"Skipping malformed persistent media descriptor cache row: "
                f"{log_preview(row)}"
            )
            continue
    ...
    logger.info(f"Hydrated {loaded_count} persistent media descriptor cache entries")
    return loaded_count
