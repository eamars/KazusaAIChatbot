...
5. `user_goal` 与 `current_blocker` 只能由当前用户表达或明确响应的语义证据支持。
   角色提出但尚未得到当前用户回应的提议、要求、条件或互动安排，可以在
   `episode_narrative`、`current_thread` 或 `character_stance` 中作为已经发生的场景事实，
   但不得写成当前用户接受、义务、目标、阻碍或直接约束判断。
   继续处理同一任务或话题不等于接受、承诺或重新打开该角色事项。只有当前用户回应、接受、拒绝、提及、询问、实质改变
   或明确重新打开时，才可以把它写入用户拥有的当前状态。
6. `overused_moves` 只记录本轮证据已经显示的重复回应模式。先将
   `accepted_turn.final_dialog` 与 `recent_turns` 中角色的近期回应，以及
   `prior_scene.overused_moves` 比较：
   - 只描述已经发生的可见回应目的或互动动作，并且必须由本轮和近期证据
     共同支持；即使措辞、意象或句式变化，仍然属于同一重复回应模式。
   - 每条记录使用紧凑的语义描述，不摘录原句，也不记录单纯的词语重复。
   - 多条模式按最近一次有证据且对当前互动影响更大的顺序排列。
   - `scene_relation` 为 `same` 或 `related` 时，只要近期证据仍支持，保留
     `prior_scene.overused_moves` 中此前已经记录的模式。
   - 如果是全新场景且没有重复模式，必须返回 `[]`。
   - 这些记录只能观察已经发生的回应，不得把重复模式改写成下一轮应该说什么、
     必须避免什么或必须采取什么行动。
7. 描述当前角色时，只能使用 `semantic_context.character_name` 的准确名字，或省略主语。
   不得用 assistant、bot、AI、角色等机器身份标签替代这个名字。
8. 对依赖今天、明天、下周等相对时间的约定、期限、计划或其他可执行事实，
...
- 当前用户输入对用户作出的选择、拒绝、纠正和承诺，以及用户对自己执行事项的
  明确完成或停止声明具有最高权威。
- 角色提出但尚未得到当前用户回应的提议、要求、条件或互动安排，只是已经发生的
  场景或历史事实，不是当前用户的意图、接受、承诺、义务或决策约束。
...
7. `relevance` 只报告当前语义作用：
   - `decision`：仍直接约束当前判断或区分已处理与未处理事项。当前用户尚未回应时，
     角色提议只能作为场景或历史事实；只有在当前用户回应、接受、拒绝、提及、询问、
     实质改变或明确重新打开该事项时，角色提议才可以写为 `decision`。
     继续处理底层任务本身不构成回应、接受或重新打开。
