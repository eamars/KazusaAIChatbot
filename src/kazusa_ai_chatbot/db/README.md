...
- conversation history: save, recent-history retrieval, semantic/keyword
  ...
  attachment-description repair; pre-queue ingress receipt commits, durable
  interleaving queries, and user-role source-row lookup;
...
Inbound user receipts committed before queue admission carry a server-only
`received_at` field: the canonical UTC arrival instant generated by the
service, never copied from adapter `local_timestamp` or any client clock. The
existing `timestamp` field keeps its event/local-time meaning and is not the
authoritative arrival clock. `save_conversation_receipt(...)` commits the row
before slow embedding enrichment and updates the same row afterward. Embedding
enrichment is best-effort after that commit: a failure leaves the durable
receipt available for history and intake ordering while emitting a warning.
Legacy rows written before this contract may lack `received_at`; they remain
valid historical records but are not authoritative evidence for delivery
promotion.
`has_inbound_after(...)` counts every user-role row in the same
platform/channel whose `received_at` is strictly after the owner receipt and
at or before a response cutoff, independent of author or later intake
disposition.
...
`save_conversation(...)` and `save_conversation_receipt(...)` require a string
`body_text` field and reject a row when that text is blank after attachment
storage policy leaves no storable attachment.
