...
are not generated by the LLM. `source_episode_refs` are canonicalized by root
episode ID when reflection runs are recursively combined: repeated messages
from one settled episode may contribute different `captured_at` values, so the
earliest normalized timestamp is retained when correlation, local date, and
scope metadata agree. A genuine metadata conflict remains fail-closed.
