...
async def test_live_facts_harvester_contract_relative_due_terms(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Facts harvester must put resolved relative due time in due_time."""

    await _skip_if_endpoint_unavailable(CONSOLIDATION_LLM_BASE_URL)
    capturing_llm = _CapturingAsyncLLM(facts_module._facts_harvester_llm)
    monkeypatch.setattr(facts_module, '_facts_harvester_llm', capturing_llm)

    state = _facts_harvester_state()
    result = await facts_module.facts_harvester(state)
    future_promises = result['future_promises']
    diagnostic = _temporal_diagnostic(
        future_promises,
        expected_absolute_dates=('2026-05-09',),
    )
    trace_path = write_llm_trace(
        'temporal_relative_terms_live_llm',
        'facts_harvester_contract_relative_due_terms',
        {
            'state': state,
            'llm_calls': capturing_llm.calls,
            ...
            'diagnostic': diagnostic,
            'judgment': (
                'Facts harvester should emit a promise with absolute due_time '
                'and no relative timing in action.'
            ),
        },
    )
    logger.info(
        f'TEMPORAL_LIVE facts_harvester trace={trace_path} '
        f'diagnostic={diagnostic} promises={future_promises}'
    )

    assert future_promises
    promise = future_promises[0]
    assert str(promise.get('due_time', '')).startswith('2026-05-09')
    assert not _relative_hits(str(promise.get('action', '')))
    assert trace_path.exists()


def _facts_evaluator_state() -> dict[str, Any]:
    """Build an evaluator fixture with a relative-time promise leak."""
    ...
    state.update({
        ...
        'future_promises': [
            {
                'target': '杏山千纱 (Kyōyama Kazusa)',
                'action': '杏山千纱明天早上提醒用户检查香料清单',
                'due_time': None,
                'commitment_type': 'future_promise',
                'dedup_key': 'spice_check_reminder',
            },
        ],
...
async def test_live_facts_evaluator_contract_relative_due_terms(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Facts evaluator must reject unresolved relative promise timing."""

    await _skip_if_endpoint_unavailable(CONSOLIDATION_LLM_BASE_URL)
    capturing_llm = _CapturingAsyncLLM(
        facts_module._fact_harvester_evaluator_llm,
    )
    monkeypatch.setattr(
        facts_module,
    ...
    state = _facts_evaluator_state()
    result = await facts_module.fact_harvester_evaluator(state)
    trace_path = write_llm_trace(
        'temporal_relative_terms_live_llm',
        'facts_evaluator_contract_relative_due_terms',
        {
            'state': state,
            'llm_calls': capturing_llm.calls,
            'result': result,
            'judgment': (
                'Facts evaluator should request correction when a promise '
                'action contains unresolved relative timing and due_time is null.'
            ),
        },
    )
    logger.info(
        f'TEMPORAL_LIVE facts_evaluator trace={trace_path} result={result}'
    )
