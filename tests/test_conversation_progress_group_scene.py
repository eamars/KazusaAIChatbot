...
from kazusa_ai_chatbot.nodes import persona_supervisor2 as persona_node
...
@pytest.mark.asyncio
async def test_persona_supervisor_reports_group_scene_success_non_group_and_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Persona owner carries group-scene text, context, and disposition."""

    fixture = _fixture()
    trigger = fixture["trigger"]

    class FakeStateGraph:
        """Run the persona owner boundary without executing semantic nodes."""
    ...
    monkeypatch.setattr(persona_node, "StateGraph", FakeStateGraph)
    monkeypatch.setattr(
        persona_node,
        "filter_group_scene_ambient_turns",
        lambda **_kwargs: fixture["ambient_logical_turns"],
    )
    monkeypatch.setattr(
        persona_node,
    ...
    monkeypatch.setattr(
        persona_node,
        "_build_scope_users",
        lambda _state, _ambient_history: fixture["scope_users"],
    )
    monkeypatch.setattr(
        persona_node,
        "_build_scene_participant_bindings",
        lambda _state, _scope_users: [],
    )
    ...
    base_state: dict[str, Any] = {
        ...
        "storage_timestamp_utc": trigger["occurred_at"],
        "local_time_context": {
            "current_local_datetime": "2026-07-14 12:00",
            "current_local_weekday": "Tuesday",
        },
        "user_input": trigger["body_text"],
        ...
        "platform": "debug",
        "platform_channel_id": "group-scene-channel",
        "channel_type": "group",
        "channel_name": "Group scene",
        "platform_message_id": "group-scene-message",
        "platform_user_id": "platform-a",
        "global_user_id": "user-a",
        "user_name": trigger["speaker_name"],
        "user_profile": {},
    ...
    async def run_owner(channel_type: str) -> dict[str, Any]:
        state = deepcopy(base_state)
        state["channel_type"] = channel_type
        return await persona_node.persona_supervisor2(state)

    expected_context = _build_from_fixture(
        fixture,
        current_global_user_id="user-a",
    )
    expected_rendered = project_group_scene_prompt(expected_context)
    success = await run_owner("group")
    success_state = success["consolidation_state"]
    assert success["public_group_scene_projection_status"] == "completed"
    assert success["public_group_scene_projection_reason"] == "available"
    assert success["public_group_scene_context"] == expected_context
    assert success_state["public_group_scene"] == expected_rendered
    assert success_state["public_group_scene_context"] == expected_context
    assert success_state["public_group_scene_projection_status"] == (
        "completed"
    )
    assert success_state["public_group_scene_projection_reason"] == (
        "available"
    )

    non_group = await run_owner("private")
    non_group_state = non_group["consolidation_state"]
    assert non_group["public_group_scene_projection_status"] == "skipped"
    assert non_group["public_group_scene_projection_reason"] == "not_group"
    assert "public_group_scene_context" not in non_group
    assert non_group_state["public_group_scene"] == ""
    assert "public_group_scene_context" not in non_group_state
    assert non_group_state["public_group_scene_projection_status"] == (
        "skipped"
    )
    assert non_group_state["public_group_scene_projection_reason"] == (
        "not_group"
    )
    ...
    failure = await run_owner("group")
    failure_state = failure["consolidation_state"]
    assert failure["public_group_scene_projection_status"] == "failed"
    assert failure["public_group_scene_projection_reason"] == (
        "projection_unavailable"
    )
    assert "public_group_scene_context" not in failure
    assert failure_state["public_group_scene"] == ""
    assert "public_group_scene_context" not in failure_state
    assert failure_state["public_group_scene_projection_status"] == "failed"
    assert failure_state["public_group_scene_projection_reason"] == (
        "projection_unavailable"
    )
