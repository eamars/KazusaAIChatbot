"""Live LLM user-wire acceptance nodes for Plan 3."""
...
def test_prerequisite_failure_trace_identity_prefers_public_turn_trace() -> None:
    """Turn 2 failure capture uses the public trace and stored turn identity."""

    response = {
        "trace_id": "llmtrace-public-turn-002",
        "operational_error": {
            "trace_id": "llmtrace-nested-operational-error",
        },
    }

    assert _plan3_authoritative_response_trace_id(response) == (
        "llmtrace-public-turn-002"
    )
    assert _plan3_turn_trace_query(
        platform="debug",
        platform_channel_id="channel-turn-002",
        platform_message_id="message-turn-002",
    ) == {
        "platform": "debug",
        "platform_channel_id": "channel-turn-002",
        "platform_message_id": "message-turn-002",
...
def test_prerequisite_pending_query_uses_only_stored_ledger_fields() -> None:
    """Turn 1 pending lookup stays within the ledger row's owned fields."""

    query = _plan3_turn_001_pending_query(
        platform="debug",
        platform_channel_id="channel-turn-001",
        global_user_id="global-user-001",
        source_message_id="message-turn-001",
    )

    assert query == {
        "action_kind": RESOLVER_PENDING_HIL_ACTION_KIND,
        "trigger_id": "message-turn-001",
        "target_scope.platform": "debug",
        "target_scope.platform_channel_id": "channel-turn-001",
        "target_scope.global_user_id": "global-user-001",
        "target_scope.source_message_id": "message-turn-001",
    }
    assert "platform_user_id" not in query
    assert "source_platform" not in query
    assert ACTION_ATTEMPT_LEDGER_COLLECTION == "self_cognition_action_attempts"


@pytest.mark.asyncio
async def test_prerequisite_operational_error_persists_trace_before_cleanup(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Failure artifacts retain protected P attempts before DB cleanup."""
    ...
    trace_id = _plan3_authoritative_response_trace_id(response)
    trace_run = {"trace_id": trace_id, "status": "failed"}
    trace_steps = [
        {
            "trace_id": trace_id,
            "sequence": 4,
    ...
    async def wait_for_trace(
        ...
        *,
        ...
        del database, timeout_seconds
        captured_trace_ids.append(observed_trace_id)
        return trace_run
    ...
    monkeypatch.setattr(
        ...
        "_wait_for_plan3_trace_finalization",
        wait_for_trace,
    )
    monkeypatch.setattr(
        sys.modules[__name__],
        "_read_plan3_trace_documents",
        read_trace,
    )

    snapshot = await _capture_plan3_failure_evidence(
    ...
    assert captured_trace_ids == ["llmtrace-public-turn-002"]
    assert snapshot["trace_id"] == "llmtrace-public-turn-002"
    assert snapshot["trace_steps"] == trace_steps
    assert json.loads((tmp_path / "failure_evidence.json").read_text())[
        "trace_steps"
    ] == trace_steps
    assert json.loads((tmp_path / "trace.json").read_text())["trace_id"] == (
        "llmtrace-public-turn-002"
    )


@pytest.mark.asyncio
async def test_prerequisite_failure_evidence_retains_turn_001_pending_contract(
    tmp_path: Path,
) -> None:
    """Failure artifacts retain the validated Turn 1 trace and HIL ledger row."""

    original_goal = (
        "Please handle this in the background. Before opening either file, ask "
        "which file to summarize."
    )
    pending_resume = {
        "schema_version": RESOLVER_PENDING_RESUME_VERSION,
        "resume_id": "resume-turn-001",
        ...
        "prompt_safe_question": (
            "Which should I summarize: plan3_real_user_e2e/alpha.txt or "
            "plan3_real_user_e2e/beta.txt?"
        ),
    ...
    pending_row = {
        ...
        "status": "waiting_for_user",
        "continuation_status": "waiting_for_user",
    ...
    validated_pending = _plan3_validate_turn_001_pending_row(
        pending_row,
        platform="debug",
        platform_channel_id="channel-turn-001",
        global_user_id="global-user-001",
    ...
    turn_001_trace_id = "llmtrace-turn-001"
    turn_001_trace_run = {
        "trace_id": turn_001_trace_id,
        "status": "succeeded",
    }
    turn_001_trace_steps = [
        {
            "trace_id": turn_001_trace_id,
            "sequence": 4,
            "stage_name": "cognition_core_v3.P",
            "raw_response_text": "{\"resolver_requests\":[{\"capability\":\"human_clarification\"}]}",
        }
    ]
    ...
    snapshot = await _capture_plan3_failure_evidence(
        ...
        turn_001_pending_rows=[pending_row],
    )

    assert snapshot["turn_001"] == {
        "trace_id": turn_001_trace_id,
        "trace_run": turn_001_trace_run,
        "trace_steps": turn_001_trace_steps,
        "pending_rows": [pending_row],
    }
    assert validated_pending["pending_task_continuation"] == {
        "schema_version": PENDING_TASK_CONTINUATION_VERSION,
        "on_answered_clarification": "background_task_admission",
    }
    failure_artifact = json.loads((tmp_path / "failure_evidence.json").read_text())
    trace_artifact = json.loads((tmp_path / "trace.json").read_text())
    assert failure_artifact["turn_001"] == snapshot["turn_001"]
    assert trace_artifact["turn_001"] == snapshot["turn_001"]


@pytest.mark.asyncio
async def test_prerequisite_failure_evidence_preserves_delivery_child_traces(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Failure artifacts retain ordered background-delivery P candidates."""

    primary_trace_id = "llmtrace-public-turn-002"
    child_trace_rows = [
        {
            "trace_id": "llmtrace-delivery-second",
            "created_at": "2026-08-31T00:00:02+00:00",
        },
        {
            "trace_id": "llmtrace-delivery-first",
            "created_at": "2026-08-31T00:00:01+00:00",
        },
    ]
    trace_runs = {
        primary_trace_id: {"trace_id": primary_trace_id, "status": "succeeded"},
        "llmtrace-delivery-first": {
            "trace_id": "llmtrace-delivery-first",
            "status": "failed",
        },
        "llmtrace-delivery-second": {
            "trace_id": "llmtrace-delivery-second",
            "status": "failed",
        },
    }
        ...
        primary_trace_id: [
            {
                ...
                "sequence": 1,
                "stage_name": "cognition_core_v3.P",
                "raw_response_text": "{\"goal_resolution\": \"answerable_now\"}",
            }
        ],
        "llmtrace-delivery-first": [
            {
    ...
    async def read_rows(
        ...
        del database
        assert collection_name == "llm_trace_runs"
        assert query == {"source_background_work_job_id": "job-delivery"}
        return child_trace_rows
    ...
    async def wait_for_trace(
        ...
        *,
        ...
        del database, timeout_seconds
        finalized_trace_ids.append(trace_id)
        return trace_runs[trace_id]
    ...
    async def read_trace(
        ...
        del database
        return trace_runs[trace_id], trace_steps[trace_id]

    monkeypatch.setattr(sys.modules[__name__], "_plan3_read_rows", read_rows)
    monkeypatch.setattr(
        sys.modules[__name__],
        "_wait_for_plan3_trace_finalization",
        wait_for_trace,
    ...
    snapshot = await _capture_plan3_failure_evidence(
        ...
        database=object(),
        trace_id=primary_trace_id,
        background_work_job_id="job-delivery",
    )

    assert finalized_trace_ids == [
        primary_trace_id,
        "llmtrace-delivery-first",
        "llmtrace-delivery-second",
    ]
    assert snapshot["trace_id"] == primary_trace_id
    assert [row["trace_id"] for row in snapshot["companion_traces"]] == [
        "llmtrace-delivery-first",
        "llmtrace-delivery-second",
    ]
    assert all(
        "pending_task_continuation" in row["trace_steps"][0]["raw_response_text"]
        for row in snapshot["companion_traces"]
    )
    trace_artifact = json.loads((tmp_path / "trace.json").read_text())
    assert trace_artifact["trace_id"] == primary_trace_id
    assert [row["trace_id"] for row in trace_artifact["companion_traces"]] == [
        "llmtrace-delivery-first",
        "llmtrace-delivery-second",
    ]
...
@pytest.mark.live_llm
@pytest.mark.asyncio
async def test_e2e_native_coding_uses_brain_approval_and_returns_verified_artifact(
    tmp_path: Path,
) -> None:
    """Prove native coding, internal approval, edit, test, and final evidence."""

    _require_plan3_inline_live_backend()
    from kazusa_ai_chatbot.db._client import get_db

    suffix = uuid4().hex
    marker = f"DSH_PLAN3_NATIVE_CODING_{suffix}"
    platform = f"dsh-plan3-native-coding-{suffix}"
    channel_id = f"dsh-plan3-native-coding-channel-{suffix}"
    user_id = f"dsh-plan3-native-coding-user-{suffix}"
    message_id = f"dsh-plan3-native-coding-message-{suffix}"
    ...
    try:
        ...
        case_root = workspace_root / "native-coding" / f"case-{suffix}"
        case_root.mkdir(parents=True, exist_ok=False)
        source_path = case_root / "calculator.py"
        test_path = case_root / "test_calculator.py"
        ...
        relative_case = case_root.relative_to(workspace_root).as_posix()
        workspace_parent = workspace_root.parent.as_posix()
        user_text = (
            "Use only the native Standard workspace tools to inspect and repair "
            f"the deterministic case {relative_case}/calculator.py. Change the "
            "implementation so its test passes, then run the test file with "
            f"the working directory {workspace_parent}. That working directory "
            "is outside the admitted workspace, so let the character-owned "
            "internal approval bridge judge that exact native operation. Use "
            "the same DSH task to verify the edit, submit one terminal result, "
            f"and include the evidence marker {marker}. Do not import or invoke "
            "any Kazusa coding executor."
        )
        request_payload = _plan3_chat_request(
            ...
            user_id=user_id,
            bot_id=bot_id,
            display_name=f"Plan 3 native coder {suffix[:8]}",
            channel_name=f"Plan 3 native coding {suffix[:8]}",
            message_id=message_id,
            text=user_text,
            no_remember=True,
        )
        ...
        assert len(approval_asked) == 1
        assert len(approval_decided) == 1
        asked = approval_asked[0]
        decided = approval_decided[0]
        approval_id = asked.get("id")
        assert isinstance(approval_id, str) and approval_id.strip()
        assert decided.get("id") == approval_id
        assert decided.get("outcome") == "allowed-once"
        grant = decided.get("grant")
        assert isinstance(grant, Mapping)
        assert grant.get("schema_version") == "dsh_brain_interaction.v2"
        assert grant.get("interaction_id") == approval_id
        assert grant.get("grant_status") == "consumed"
        assert isinstance(grant.get("activation_id"), str)
        forbidden_interactions = {
            "userQuestions/asked",
            "userQuestions/answered",
            "dsh_interaction_pending",
        }
        assert not forbidden_interactions & {
            event_type for _, event_type, _ in dsh_events
        }
        ...
        assert any(
            name != "submit_resolution" and not name.startswith("kazusa_")
            for name in call_names
        )
        assert "calculator.py" in json.dumps(tool_calls, ensure_ascii=False)
        terminal_events = [
            event
            for event in tool_results
            if isinstance(event.get("meta"), Mapping)
        ...
        assert len(terminal_events) == 1
        terminal_json = json.dumps(terminal_events[0], ensure_ascii=False)
        assert marker in terminal_json
        assert "kazusa_ai_chatbot.coding_agent" not in terminal_json
        assert "complex_task_resolver" not in terminal_json
        native_output = "\n".join(
            child.stdout_path.read_text(encoding="utf-8", errors="replace")
            + child.stderr_path.read_text(encoding="utf-8", errors="replace")
            for child in child_processes
        )
        assert "kazusa_ai_chatbot.coding_agent" not in native_output
        assert "complex_task_resolver" not in native_output
        assert source_path.read_text(encoding="utf-8") == (
            "def add(left: int, right: int) -> int:\n"
            "    return left + right\n"
        )
        local_test = await asyncio.to_thread(
            subprocess.run,
            [str(PYTHON_EXECUTABLE), "-m", "pytest", "-q", str(test_path)],
            cwd=case_root,
            capture_output=True,
            text=True,
            check=False,
        ...
        _write_json(
            artifact_dir / "native_test_result.json",
            {
                "returncode": local_test.returncode,
                "stdout": local_test.stdout,
                "stderr": local_test.stderr,
            },
        )
        assert local_test.returncode == 0
        ...
        conclusion_lines = [
            ...
            "",
            f"- Marker: `{marker}`",
            f"- Visible result captured: `{bool(visible_text)}`",
            f"- Internal approval events: `{len([event for _, event_type, event in dsh_events if event_type == 'approval/decided'])}`",
            f"- Terminal receipts: `{len([event for _, event_type, event in dsh_events if event_type == 'tool/result' and isinstance(event.get('meta'), Mapping) and isinstance(event['meta'].get('kazusa'), Mapping) and event['meta']['kazusa'].get('kind') == 'terminal_resolution_v2'])}`",
            f"- Trace retained: `{bool(trace_id)}`",
            f"- Duration ms: `{round((time.perf_counter() - started_at) * 1000)}`",
            "- Judgment: native Standard tools own the edit and test; the Brain-owned V2 approval is internal, one-shot, and followed by terminal evidence and ordinary character delivery.",
        ]
        (artifact_dir / "behavior_audit_conclusions.md").write_text(
            "\n".join(conclusion_lines) + "\n",
            encoding="utf-8",
        ...
        print(f"DSH_PLAN3_NATIVE_CODING_ARTIFACT={artifact_dir}")


@pytest.mark.live_llm
@pytest.mark.asyncio
async def test_e2e_coding_followups_revision_summary_status_approval_blocker_and_cancel_preserve_session(
    tmp_path: Path,
) -> None:
    """Exercise opaque task controls and internal DSH decisions on one session."""

    _require_plan3_inline_live_backend()
    from kazusa_ai_chatbot.db._client import get_db

    suffix = uuid4().hex
    marker = f"DSH_PLAN3_CODING_CONTROLS_{suffix}"
    platform = "debug"
    channel_id = f"plan3-coding-controls-channel-{suffix}"
    user_id = f"plan3-coding-controls-user-{suffix}"
    bot_id = os.environ["CHARACTER_GLOBAL_USER_ID"].strip()
    artifact_dir, artifact_stamp = _plan3_artifact_directory(
        "coding_followups",
        suffix,
    ...
    turns = (
        (
            "initial",
            (
                "In the task workspace, create a small deterministic coding task "
                f"that writes the marker {marker} and verifies it with a native "
                "test. Run it as a background task, keep it resumable, and "
                "deliver its result when complete."
            ),
        ),
        (
            "revise",
            (
                "Revise the same accepted task so its verification also records "
                "the bounded marker in the final artifact."
            ),
        ),
        (
            "summarize",
            (
                "Summarize the same accepted task and report only its current "
                "prompt-safe progress."
            ),
        ),
        (
            "status",
            "What is the current status of the same accepted task?",
        ),
        (
            "approval-question-review",
            (
                "Continue the same task with the exact safe verification "
                "operation. Let any execution-time approval, question, or plan "
                "review be decided internally by the character, and retain a "
                "bounded blocker if a required operation is unavailable."
            ),
        ),
        (
            "cancel",
            (
                "Cancel the same accepted task and close its current continuation "
                "without starting a different task."
            ),
        ),
    )
    runtime_environment: dict[str, str] | None = _runtime_environment(tmp_path)
    ...
    callback_secret = f"plan3-coding-controls-callback-{suffix}"
    callback_server = _Plan3LoopbackAdapterServer(
        platform=platform,
        shared_secret=callback_secret,
    )
    callback_started = False
    started_at = time.perf_counter()
    final_binding_rows: list[dict[str, Any]] = []
    final_task_rows: list[dict[str, Any]] = []
    final_job_rows: list[dict[str, Any]] = []
    dsh_events: list[tuple[int, str, dict[str, Any]]] = []
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            assert registration_response.status_code == 200
            for index, (kind, text) in enumerate(turns, start=1):
                message_id = f"plan3-coding-controls-{kind}-{suffix}"
                request_payload = _plan3_chat_request(
                    platform=platform,
                    channel_id=channel_id,
                    user_id=user_id,
                    bot_id=bot_id,
                    display_name=f"Plan 3 coding controls {suffix[:8]}",
                    channel_name=f"Plan 3 coding controls {suffix[:8]}",
                    message_id=message_id,
                    text=text,
                    no_remember=False,
                )
                ...
                _write_json(
                    artifact_dir / f"turn_{index:03d}_request.json",
                    request_payload,
                )
                response_payload = await _post_plan3_chat(
                    client,
                ...
                _write_json(
                    artifact_dir / f"turn_{index:03d}_response.json",
                    response_payload,
                )
                visible_text = _plan3_visible_text(response_payload)
                (artifact_dir / f"turn_{index:03d}_log.txt").write_text(
                    f"kind={kind}\nvisible_text={visible_text}\n",
                    encoding="utf-8",
                )
                _write_json(
                    artifact_dir / f"turn_{index:03d}_callbacks.json",
                    callback_server.delivery_payloads(),
                )
                assert response_payload.get("operational_error") is None
                if index == 1:
                    await _wait_for_plan3_scoped_rows(
                        database,
                        collection_name="accepted_tasks",
                        query={
                            "schema_version": "accepted_task.v2",
                    ...
                    await _wait_for_plan3_deliveries(
                        callback_server,
                        count=1,
                        channel_id=channel_id,
                    )
                elif kind in {"revise", "approval-question-review"}:
                    expected_task_count = 2 if kind == "revise" else 3
                    await _wait_for_plan3_scoped_rows(
                        database,
                        collection_name="accepted_tasks",
                        query={
                        ...
                        minimum_count=expected_task_count,
                    )
                    await _wait_for_plan3_deliveries(
                        callback_server,
        ...
        final_task_rows = await _plan3_read_rows(
            database,
            "accepted_tasks",
            {
                "schema_version": "accepted_task.v2",
        ...
        final_job_rows = await _plan3_read_rows(
            database,
            "background_work_jobs",
            {
                "schema_version": "background_work_job.v2",
        ...
        final_binding_rows = await _plan3_read_rows(
            database,
            "dsh_task_bindings",
            {
                "schema_version": "dsh_task_binding.v1",
        ...
        assert final_task_rows
        assert final_job_rows
        assert len(final_binding_rows) == 1
        binding = final_binding_rows[0]
        task_session_ids = {
            row.get("dsh_task_session_id")
            for row in final_task_rows
            if isinstance(row.get("dsh_task_session_id"), str)
        }
        task_session_ids.add(binding.get("task_session_id"))
        task_session_ids.discard(None)
        assert len(task_session_ids) == 1
        session_id = next(iter(task_session_ids))
        ...
        interaction_types = {
            event_type
            for _, event_type, _ in dsh_events
            if "interaction" in event_type or "question" in event_type
        }
        assert not {
            "userQuestions/asked",
            "userQuestions/answered",
        ...
        interaction_rows = await _plan3_read_rows(
            database,
            "dsh_interaction_store",
            {"resolution_thread_id": thread_id},
        )
        assert all(
            row.get("schema_version") == "dsh_brain_interaction.v2"
            for row in interaction_rows
        )
        generations = [
            row.get("dsh_operation_generation")
            for row in final_task_rows
            if isinstance(row.get("dsh_operation_generation"), int)
        ]
        assert generations == sorted(generations)
        assert len(set(generations)) >= 2
        event_text = json.dumps(dsh_events, ensure_ascii=False)
        assert "coding_run" not in event_text
        assert "pending_user" not in event_text
        assert "relay_to_user" not in event_text
        trace_candidates = await _plan3_read_rows(
            database,
            "llm_trace_runs",
            {"platform": platform, "platform_channel_id": channel_id},
        )
        for candidate in trace_candidates:
            candidate_trace_id = candidate.get("trace_id")
            if not isinstance(candidate_trace_id, str) or not candidate_trace_id:
                continue
            finalized = await _wait_for_plan3_trace_finalization(
                database,
                candidate_trace_id,
            )
            _, steps = await _read_plan3_trace_documents(
                database,
                candidate_trace_id,
            )
            trace_rows.extend(steps)
            trace_id = candidate_trace_id
            assert finalized.get("status") == "succeeded"
        assert trace_rows
        _write_json(
            artifact_dir / "dsh_execution_lineage.json",
            {
                "task_session_id": session_id,
                "resolution_thread_id": thread_id,
                "segment_id": segment_id,
                "accepted_tasks": final_task_rows,
                "background_work_jobs": final_job_rows,
                "bindings": final_binding_rows,
        ...
        _write_json(
            artifact_dir / "trace.json",
            {"trace_id": trace_id, "steps": trace_rows},
        )
    finally:
        failure_snapshot = await _capture_plan3_failure_evidence(
            artifact_dir=artifact_dir,
            ...
            known_binding=(binding if isinstance(binding, Mapping) else None),
            known_dsh_events=dsh_events,
            known_trace_steps=trace_rows,
            callback_payloads=callback_server.delivery_payloads(),
        )
        if not trace_id and isinstance(failure_snapshot.get("trace_id"), str):
            trace_id = failure_snapshot["trace_id"]
        if not trace_rows and isinstance(failure_snapshot.get("trace_steps"), list):
            trace_rows = list(failure_snapshot["trace_steps"])
        if not dsh_events and isinstance(failure_snapshot.get("dsh_events"), list):
            dsh_events = list(failure_snapshot["dsh_events"])
        if callback_started:
        ...
        _write_json(
            artifact_dir / "readiness.json",
            readiness or {},
        )
        _write_json(
            artifact_dir / "callbacks.json",
            callback_server.delivery_payloads(),
        )
        conclusion_lines = [
            "# Coding follow-up lifecycle acceptance",
            "",
            f"- Turns captured: `{len(responses)}`",
            f"- Final callback count: `{len(callback_server.delivery_payloads())}`",
            f"- One binding observed: `{len(final_binding_rows) == 1}`",
            f"- Trace evidence retained: `{bool(trace_id)}`",
            f"- Duration ms: `{round((time.perf_counter() - started_at) * 1000)}`",
            "- Judgment: all controls are sent through ordinary public chat with one stable identity; durable task controls use opaque refs while DSH internal decisions remain inside the V2 cognition bridge.",
        ]
        (artifact_dir / "behavior_audit_conclusions.md").write_text(
            "\n".join(conclusion_lines) + "\n",
            encoding="utf-8",
        ...
        print(f"DSH_PLAN3_CODING_FOLLOWUPS_ARTIFACT={artifact_dir}")
