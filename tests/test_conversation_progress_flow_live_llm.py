...
def _base_state() -> dict:
    """Build the direct content-plan/dialog state for the flow diagnostic.
...
async def _judge_flow(state: dict, dialog: dict) -> dict[str, Any]:
    ...
    Args:
        state: Final state containing the prompt inputs and content plan.
...
def _continuity_metrics(content_plan: str, final_dialog: list[str]) -> dict[str, Any]:
    ...
    Args:
        content_plan: content plan generated by the live Content Plan
            agent.
...
def _release_case_state(case: dict[str, Any]) -> dict:
    """Build a direct content-plan/dialog state for a release flow case.
    ...
    Returns:
        Mutable state ready for Content Plan.
    """
...
async def test_live_flow_baseline_thesis_contribution_case(ensure_live_llm) -> None:
    ...
    continuity_metrics = _continuity_metrics(content_plan, final_dialog)
    trace_payload = {
        "variant": _TRACE_VARIANT,
