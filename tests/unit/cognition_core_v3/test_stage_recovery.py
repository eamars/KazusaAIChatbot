...
@pytest.mark.asyncio
async def test_top_level_task_background_flag_regenerates_to_nested_request(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """P keeps an invalid task flag recorded until a canonical retry replaces it."""
    ...
    token = facade_module.bind_protected_chain_records(run_id="task-flag-recovery")
    try:
        result = await _run_cognition_stage(
            services=_services(invoker),
            stage="P",
    ...
    assert result.resolver_requests == tuple(canonical_plan["resolver_requests"])
    assert invoker.calls == 2
    repair = json.loads(invoker.messages[1][1].content)["contract_repair"]
    assert repair["contract_error"] == (
        "response plan: unexpected fields ['task_resolution_start_in_background']"
    )
    ...
    assert [row["status"] for row in records] == ["contract_fault", "parsed"]
    assert [row["parse_status"] for row in trace_rows] == [
        "contract_error",
        "succeeded",
    ]
...
@pytest.mark.asyncio
async def test_pending_background_continuation_mismatch_regenerates_to_preserved_value(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """P records the invalid timing candidate and accepts its canonical retry."""
    ...
    pending_continuation = {
        ...
        "original_goal": "Complete the task in the background after clarification.",
    ...
    invalid_plan = {
        ...
        "resolver_requests": [{
            "capability": "task_resolution_request",
            "goal": "complete the bounded task",
            "reason": "the answered task needs evidence",
            "start_in_background": False,
        }],
        "epistemic_boundary": "The evidence remains source-owned.",
        "pending_resolution": {
            "decision": "answered",
            "reason": "The current observation answers the clarification.",
        },
    }
    canonical_plan = {
        **invalid_plan,
        "resolver_requests": [{
            **invalid_plan["resolver_requests"][0],
            "start_in_background": True,
        }],
    }
    invoker = _SequencedInvoker([invalid_plan, canonical_plan])
    token = facade_module.bind_protected_chain_records(
        run_id="pending-timing-recovery",
    )
    try:
        result = await _run_cognition_stage(
            services=_services(invoker),
...
@pytest.mark.asyncio
async def test_pending_plan_carrier_echo_regenerates_to_pending_variant(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """P retries when a pending-resolution result echoes its input carrier."""
    ...
    invoker = _SequencedInvoker([invalid_plan, canonical_plan])
    token = facade_module.bind_protected_chain_records(
        run_id="pending-variant-recovery",
    )
    try:
        result = await _run_cognition_stage(
            services=_services(invoker),
...
@pytest.mark.asyncio
async def test_post_pending_task_echo_regenerates_without_duplicate_admission(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Post-answer P retries instead of admitting the completed task again."""

    trace_rows = _trace_capture(monkeypatch)
    invalid_plan = {
        "response_goal": "acknowledge that the task is continuing",
        "action_requests": [],
        "epistemic_boundary": "The evidence remains source-owned.",
        "goal_resolution": "requires_required_evidence",
        "resolver_requests": [{
            ...
            "reason": "the prior admission was echoed",
            "start_in_background": True,
        }],
    }
    canonical_plan = {
    ...
    invoker = _SequencedInvoker([invalid_plan, canonical_plan])
    token = facade_module.bind_protected_chain_records(
        run_id="post-pending-task-recovery",
    )
    try:
        result = await _run_cognition_stage(
            services=_services(invoker),
